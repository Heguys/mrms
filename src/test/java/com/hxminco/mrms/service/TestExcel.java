 ...
 */
 ...
 class TestExecl {
    ...
    public static boolean marchers(String str) {
    ...
    public static void main(String[] args) throws Exception {
        List<String> fileList = readString(new File("E:\\MyWork\\IDEWorkSpace\\mrms\\LIST1.TXT"));
        Set<String> ids = readSet(new File("E:\\MyWork\\IDEWorkSpace\\mrms\\ID.txt"));
        System.out.println(ids.size());
        String fileName = null;
        String filePrefix = "C:\\Users\\Employee\\Desktop\\新20170915\\新20170915\\AllExcel\\";
        TestExecl poi = new TestExecl();
        List<List<String>> value = new ArrayList<>();
        List<String> key = new ArrayList<>();
        for (String one:fileList){
            fileName = filePrefix+one;
            ...
            if (list != null) {
                for (int i = 0; i < list.size(); i++) {
                    ...
                    for (int j = 0; j < cellList.size(); j++) {
                        if(marchers(cellList.get(j)) ){
                            num = 1;
//                            value.add(cellList);
                            key.add(cellList.get(j));
